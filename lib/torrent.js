...
class Torrent extends EventEmitter {
  ...
  _onWire (wire, addr) {
    ...
    wire.on('download', downloaded => {
      ...
      this.emit('download.stats', { wire, length: downloaded })
      this.client.emit('download', downloaded)
      this.client.emit('download.stats', { wire, length: downloaded })
    })
    ...
    wire.on('upload', uploaded => {
      ...
      this.emit('upload.stats', { wire, length: uploaded })
      this.client.emit('upload', uploaded)
      this.client.emit('upload.stats', { wire, length: uploaded })
    })
